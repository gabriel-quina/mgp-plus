...
@section('content')
    <header class="d-flex align-items-center justify-content-between mb-4">
        ...
        <div class="d-flex gap-2">
            {{-- Tela só de visualização: sem botão de criar/editar/excluir --}}
        </div>
    </header>
    ...
    <form method="GET" action="{{ route('enrollments.index') }}" class="row g-2 mb-3">
        ...
        <div class="col-md-2">
            <select name="school_id" class="form-select">
                ...
                @foreach ($schoolsForFilter ?? [] as $id => $name)
                    <option value="{{ $id }}" @selected($sc == $id)>{{ $name }}</option>
                @endforeach
            </select>
        </div>
                ...
                <table class="table table-sm table-striped align-middle mb-0">
                    ...
                    <tbody>
                        @forelse($enrollments as $en)
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="text-center text-muted">Nenhuma matrícula encontrada</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
