...
@php
  ...
  $lockAcademicFields = $lockAcademicFields ?? false;
  $selectedGrades = isset($selectedGrades)
      ? $selectedGrades
      : (array) old('grade_level_ids', isset($classroom) ? ($classroom->grade_level_ids ?? []) : []);
@endphp

@if ($lockAcademicFields)
  <div class="alert alert-warning">
    Esta turma já possui aulas ou avaliações. Campos acadêmicos estão bloqueados para edição.
  </div>
@endif
  ...
  <div class="card-body">
    ...
    <div class="row">
      <div class="col-md-6 mb-3">
        ...
        @if ($lockAcademicFields)
          <input type="hidden" name="workshop_id" value="{{ old('workshop_id', $classroom->workshop_id ?? null) }}">
          <div class="form-control-plaintext">{{ $classroom->workshop?->name ?? '—' }}</div>
        @else
      ...
      <div class="col-md-3 mb-3">
        ...
        @if ($lockAcademicFields)
          ...
          <div class="form-control-plaintext">
            @if ($shiftVal === 'morning')
              Manhã
            @elseif($shiftVal === 'afternoon')
              Tarde
            @elseif($shiftVal === 'evening')
              Noite
            @else
              —
            @endif
      ...
      <div class="col-md-3 mb-3">
        ...
        @php $yearVal = old('academic_year_id', isset($classroom) ? $classroom->academic_year_id : ($defaultYear ?? date('Y'))); @endphp
        @if ($lockAcademicFields)
          <input type="hidden" name="academic_year_id" value="{{ $yearVal }}">
          <div class="form-control-plaintext">{{ $yearVal }}</div>
        @else
    ...
    <div class="row">
      <div class="col-md-3 mb-3">
        ...
        @php $groupVal = old('group_number', isset($classroom) ? $classroom->group_number : 1); @endphp
        @if ($lockAcademicFields)
          <input type="hidden" name="group_number" value="{{ $groupVal }}">
          <div class="form-control-plaintext">{{ $groupVal }}</div>
        @else
          <input type="number" name="group_number" min="1"
                 class="form-control @error('group_number') is-invalid @enderror"
                 value="{{ $groupVal }}" required>
...
<div class="card mb-3">
  ...
  <div class="card-body">
    @if ($lockAcademicFields)
      ...
      <div class="form-control-plaintext">
        @foreach ($selectedGrades as $id)
          <span class="badge bg-secondary me-1">{{ $gradeLevels[$id] ?? '—' }}</span>
        @endforeach
      </div>
    @else
